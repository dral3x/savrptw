...
//	}
	...
	protected static VRPTWSolution annealing_step(VRPTWSolution start_solution, double temperature) {
		...
		// prendo il cliente pi� vicino alla route selezionata
		...
		//System.out.println("il customer pi� vicino � "+nearestCustomer);
		...
		// TODO: non è un po' un controsenso? cerchi un custumer vicino alla rotta ma se non trovi posto per inserirlo ne crei una nuova (quando l'obiettivo è minimizzare le rotte)
		if (!inserted) {
...
//		} catch (InterruptedException e) { }
		...
		if (cost_old < cost_new && (Math.random()*(temperature/(temperature + VRPTWParameters.delta)) < 0.5)) {
			// rifiuto la nuova soluzione pi� cara della vecchia
			newSolution = start_solution;
		}
